...
=== "RTCP Reports"
    ...
    + **MOS (Mean Opinion Score)**: Its a numerical representation of perceived voice quality, modeled to approximate what a human listener would rate the call. <br>**Scale**</br><br>1.0 → Poor (unusable)</br><br>3.0 → Fair (noticeable issues)</br><br>4.0+ → Good to Excellent</br><br>5.0 → Ideal (theoretical maximum)</br>
    ...
    * **Network Statistics**: It represents **RTCP-derived network performance metrics** for the RTP media stream. These metrics describe the **health and stability of the network path** between the source and destination during the call.
    ...
    + **SSRC Details**: SSRC (Synchronization Source Identifier) is a unique identifier used in RTP and RTCP to distinguish individual media streams within a session. <br>In ConnexCS, SSRC details are used to track, correlate, and analyze media streams across RTCP reports.</br> You can click on a ssrc to check the details of a specific media stream within a session.
...
=== "Class 5"
    ...
    The **Class 5** tab displays application-level events generated during Class 5 call processing.
    
    These logs provide visibility into interactions between the call processing engine and Class 5 services, applications, and workflows.
    
    Class 5 logs are primarily used for troubleshooting feature execution, validating request processing, and analysing application behaviour during a call lifecycle.

    | Field     | Description |
    | --------- | ----------- |
    | **Time**  | Timestamp indicating when the event was generated|
    | **Event** | The event type generated by the Class 5 service or application|
    | **Value** | Event-specific data associated with the log entry|

=== "AI Logs"

    The **AI Logs** tab provides detailed execution logs for AI-powered call processing.

    It exposes the complete AI pipeline for each conversation turn, allowing administrators to inspect request flow, execution performance, model responses, and component-level processing.

    AI Logs are intended for diagnostics, performance analysis, prompt validation, and troubleshooting of AI-driven call handling workflows.

    **Conversation Structure**

    * AI interactions are organised into **Conversations** and **Turns**.
    * A **Turn** represents a single interaction cycle between the caller and the AI agent and may include multiple processing stages.

    **Processing Stages**

    | Component | Description  |
    | ----------|--------------|
    | **Speech to Text (STT)** | Transcribes caller audio into text for downstream AI processing|
    | **AI Agent (LLM)** | Executes prompt processing, context evaluation, reasoning, and response generation. |
    | **Text to Speech (TTS)** | Synthesises the generated response into audio for playback to the caller|

    The execution time displayed beside each component represents the processing duration for that stage.

    **AI Agent Execution Details**: Selecting an **AI Agent (LLM)** event displays detailed execution information for that request.

    **Performance Metrics**

    | Metric | Description |
    | -------|------------ |
    | **TTFB** | Time To First Byte. Measures the time taken for the model to begin streaming a response|
    | **Latency**     | Total request processing time from submission to completion|
    | **Environment** | Execution environment used for processing the request|

    **Execution Views**:

    | View | Description |
    | -----|------------ |
    | **Preview**| Displays formatted request and response payloads.                                              |
    | **Tools**| Displays tool calls, function executions, and external integrations invoked during processing. |
    | **Log View** | Displays detailed execution logs and diagnostic information|
    | **JSON**| Displays the raw request and response payloads in JSON format|

    **Logged Data**:

    * **Input**: Contains the prompt payload submitted to the AI model, including caller utterances, conversation history, system instructions, and contextual data used during inference.

    * **Output**: Contains the response generated by the AI model for the selected execution.

    * **Metadata**: Contains execution attributes and request metadata, including configuration parameters, processing information, and model-specific details.
