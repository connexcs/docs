# Customer Support
ConnexCS doesn't charge for support, but there are reasonable level of expectations from both parties involved. (Unless you are paying for dedicated support NOC (Network Operations Center) on Demand).
...
## Support Channels
Several methods of contacting ConnexCS for assistance are outlined below. If you need to Report a Problem, please make sure you check that section for information on issue classification and response times, and some tips for reporting issues so we can best address your needs. 
...
!!! error "When to call the support number"
    Our support line rings directly to our engineers. We kindly ask you to ring this number only during 9:00 am - 23:00 pm UTC, unless there is a CRITICAL issue.
...
!!! tip "Paid Support Escalation"
    If you have a P3, P4, we're out of working hours and you have a problem that internally you consider to be critical, you have the option to escalate it. We will charge $150 per hour for our support (min 1 hour). A P5 feature request may be escalated but a project cost may be involved, please discuss this with us if you are considering it.

!!! warning "Classification usage"
    These Severity levels are here to assess the impact of a problem but aren't followed simply. We will try to answer support requests 24/7, but repeated instances of misreporting incidents as Critical beyond normal working hours may result in fines on the account or account termination.
...
+ All other Platform (non-Critical) and Operational Issues = 8:00 am - 8:00 pm UTC business hours support. Whenever possible, we strive to provide best-effort support outside these times if we are available. We're a small team, so we request you keep this in mind when reporting issue severity. 
...
!!! tip "Issues with new interconnects"
    Issues encountered when turning up a new interconnect which isn't working aren't considered Critical. 
...
!!! failure "Respectful communications"
    We're also happy to talk with you about wider SIP topics outside of our switch where possible. Under no circumstance will we tolerate rude behaviour, intimidation, or threats. We're all SSVVP and SSCA qualified, and whenever possible, we will cite references or point you to SIP traces for fact-checking information.
...
2. Allow an adequate amount of time for us to assess the problem and resolve it. We first find if the problems are connected, adding extra time onto finding the roots themselves.
3. Call instead of sending an email. This helps us to get a concise reading on all concerns quickly and ask follow-up questions on the spot.
