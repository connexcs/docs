...
&emsp; ![alt text][compile1] Stale (needs to be compiled)

&emsp; ![alt text][compile2] In sync

!!! tip "Reasons a card may be marked Stale"
    A Rate Card is marked Stale when the information presented is NOT the same as the live/active revision. Some common examples of this are when the customer generates Customer cards from Carrier cards (preferred method) OR they modify a carrier card. In both cases, the cards is marked Stale and will need to be compiled. 
...
2. Complete required fields. (For details, see [**Configure Customer Rate Card**](https://docs.connexcs.com/customer-ratecard/#configure-customer-rate-card) below.)
3. Click **`Save`**.
...
This is the preferred method and requires you to select a Provider Rate Card in Properties > Basic. Click **`Build`** and confirm. A notification will tell you if the build was successful. You may need to refresh the card to see the new settings. 
...
**Email**: This feature allows providers to email rate cards to customers as an attachment. It's a convenient way to send customers official information relevant to their accounts. Click **`Email`**, enter the email address and click **`Send`**.
...
+ **Edit Revision**: Make inline edits to the current revision. Upon `Save`, you can select to make this new revision Active. This will keep previous revision and the changes made. 
...
* **Profits** lets you to configure profit rules. This is most useful on cards with a high number of rules.
...
!!! note "Profit Type"
    Using Absolute and Relative can be useful, especially when dealing with large cards and your providers prices change; with a single click you can regenerate the customer rate cards.
...
+ **Rate Cards**: Associate the customer to one or more Provider Cards, this will define how customer routing is built and how customer prices are derived.
...
* **Force Presentation**: Find how many rows to display for rates, this is visible to the customer: 
...
!!! tip "ConnexCS LRN Database"
    ConnexCS maintains an inhouse LRN database. This ensures a quick response time and the rates are update daily. No charges are associated for using this service, so customers have unlimited dips into the database. 

+ **Strategy**: Manage how calls are distributed across Carriers when a Customer Card is associated with multiple Provider Cards. 

!!! info "Routing Strategy"
    The Strategy selected in the Customer Card is overwritten by the global Routing Strategy. For description of each Strategy and configuration for a comprehensive Routing Strategy, see [**Routing Strategy**](https://docs.connexcs.com/routing-strategy/) . 
...
* **Rounding Method**: Specify how to handle the n+1 digit (e.g if your card is billed to 4 decimal places, this cares about the 5th digit). For example: 0.1234**5** (rounded to 4 decimal places):
...
* **Reseller**: Select Resellers to associate with this card. When the Reseller logs into the Portal, they see the settings from the perspective of a Provider Card, even though they are configured as a Customer Card. 
...
### Manual Ordering tab
This tab is only available if you enable "Manual Ordering" in **Properties :material-menu-right: Advanced**. For each Prefix you can manually order the Provider Rate Cards for a high degree of control over how calls are routed. 
