# DID
A **Direct Inward Dial (DID)** number is one that exists on the public telephone network. When dialed, the carrier delivers through the platform and on to you, based on the settings configured here. This allows inbound calls to bypass a PBX or other routing to connect directly to the destination number. 

Edit **DID (Direct Inward Dial)**, either using `Bulk Edit` or individually. 
