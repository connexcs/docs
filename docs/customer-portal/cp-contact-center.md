...
3. A Window will pop-up with two sections:
   ...
   3. Click on `See More` in the `Details` column.
      1. A `Campaign Overview` will open. This interface is used for real-time monitoring of campaigns, lead stats, agent statuses, and calling activity. <img src= "/customer-portal/img/cc45new.png">
         ...
         6. `Agents Chart`: Shows the status of agents.
      ...
      2. `Campaign Agents`: List of agents assigned to the campaign.
      ...
      3. `Lead Stats`: It shows you the total number of leads loaded into the campaign and what stage they're currently at during the calling process.
      ...
      4. `Agents in Queue`: Displays a list of agents who are waiting for a call or are ready to be assigned a call in the campaign.

4. **Agents**: This section has information on the agents. You can also `Create Bulk Agents` . <img src= "/customer-portal/img/cc16new.png">
      1. Select a `SIP Profile`.
      2. Add the `Number of Agents` for participation in this campaign.
      3. Click on `Create`. <img src= "/customer-portal/img/cc17new.png">
...
5. **Script**: The Script section enables you to create structured call scripts for agents to follow during customer interactions.
      1. Click on `Add a new Script`. <img src= "/customer-portal/img/cc18new.png">
      2. Enter the `Script Name` and `Script`.
      3. Click `Confirm` to save the script. <img src= "/customer-portal/img/cc19new.png">
...
6. **Leadset**: This section has information on the Leadsets. You can also `Create Leadset` . <img src= "/customer-portal/img/cc21new.png">
7. A new window will appear, prompting you to enter the required information:
      ...
      5. Click on `Confirm` to create the leadset. <img src= "/customer-portal/img/cc22new.png">
      6. Click on `See More` in the `Details` column.

8. **A Leadset Dashboard** monitoring window will appear which displays the metrics of the leadset. <img src= "/customer-portal/img/cc23new.png">
   1. Description of the fields in the stats window:
      ...
      5. Upload your data via `File`, `URL` or by entering the `Text`.
         1. **File**: Drag and Drop a file or click to upload the `.csv`. <img src= "/customer-portal/img/cc25new.png">
            1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc26new.png">
            2. `Set row`: To indicate where the upload should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">
            3. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc28new.png"> <img src="/customer-portal/img/cc29new.png">
            4. The successful upload can be seen in the image. <img src="/customer-portal/img/cc36new.png">
            5. The `Leadset Dashboard` is also modified. <img src= "/customer-portal/img/cc37new.png">

         2. **Uploading via URL**: Enter the URL in the CSV format and hit the Upload button. <img src="/customer-portal/img/cc29new.png">
            1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc26.png">
            2. `Set row`: To indicate where the uploaad should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">
         ...
         3. **Uploading via Text**:
            ...
            3. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc26new.png">

            4. `Set row`: To indicate where the uploaad should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">

            5. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Key`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc28new.png"> <img src="/customer-portal/img/cc29new.png">

            6. The successful upload can be seen in the image. <img src="/customer-portal/img/cc36new.png">

            7. The `Leadset Dashboard` is also modified. <img src= "/customer-portal/img/cc37new.png">
   ...
   2. **Edit Leadset**: Modify the details of an existing Leadset. The system will present the same interface as `Create Leadset`.
   3. **Add Lead (Form)**: Manually add a lead via a form. <img src="cc31.png">
   4. **Lead Status**: Displays whether a lead is available, being worked on, needs follow-up, or is no longer active.
   ...
   5. **Call Status**: Displays counts for different call outcomes; like `Call Answered`, `Failed Calls`, `Call In Progress`, `Call Ringing`, `Dropped Calls`.
   6. **Disposition Status**: Shows a pie chart for all the call disposition outcomes. Each colored section of the pie chart corresponds to one of these dispositions.
   7. **Lead Agents Activity**: It displays performance stats of individual agents. The metrics include `Call Transfered`, `Busy`, `Call Back`, `Do Not Call`, `Not interested`, `Null`, `Sale made`, `Wrong details` etc.

#### Auto Dialer

1. Navigate to **Customer Portal :material-menu-right: Contact Center**.
2. In the `Campaign` tab, click `Add Campaign`. <img src= "/customer-portal/img/cc14new.png">
3. A Window will pop-up with two sections:
   1. **Basic Info**:
      1. **Name**: Enter the Campaign name.
   ...
   2. **Config**:
      ...
      2. **Source Status**: Select `Ready`/`New` or Custom Source Status for call origination. If no status is selected the calls won't be sent.
      3. **Outbound Multiplier**: It controls how many outbound calls your system (or agent/reseller) can make at the same time.
      4. **Custom Disposition**: A user-defined set of call outcome labels that agents are required to select after each call, enabling structured tracking, systematic reporting, and optimized follow-up workflows. <img src= "/customer-portal/img/cc44new.png">
         1. `Agent Unavailable`: Call was made but hung up due to agent being busy (mostly in auto dialers).
         2. `Answered`: Call is connected.
         3. `Connection Error`: Originate method failed to work.
         4. `Error`: An error occurred at the code level.
...
4. Click on `See More` in the `Details` column.
   ...
   5. `Campaign Stats`: Provides real-time visual insights into the current status of the campaign's calling activities and agent activities.
      ...
      4. `Agents Chart`: Shows the status of agents.
      5. `Call Active`: How many agents are currently on a call.
      6. Agent Waiting`: How many agents are idle (ready and waiting for a call).
   6. `Campaign Agents`: List of agents assigned to the campaign.
   7. `Manage Agents`: Allows you to add/remove agents from the campaign from the drop-down menu. Click `Confirm`. <img src= "/customer-portal/img/cc34new.png">
   8. For example: The select agents will be added.<img src= "/customer-portal/img/cc35.png">
   9. `Refresh Button`: To update agent statuses.
   10. `Lead Stats`: It shows you the total number of leads loaded into the campaign and what stage they're currently at during the calling process.
       1. `Total Leads`: Everyone you plan to call.
       2. `Dialable Leads`: How many you still can call.
       3. `In Progress`: Live calls happening now.
       4. `Call Answered`: People who actually picked up.
...
5. **Agents**: This section has information on the agents. You can also `Create Bulk Agents` . <img src= "/customer-portal/img/cc16new.png">
      1. Select a `SIP Profile`.
      2. Add the `Number of Agents` for participation in this campaign.
      3. Click on `Create`. <img src= "/customer-portal/img/cc17new.png">
6. **Script**: The Script section enables you to create structured call scripts for agents to follow during customer interactions.
      1. Click on `Add a new Script`. <img src= "/customer-portal/img/cc18new.png">
      2. Enter the `Script Name` and `Script`.
      3. Click `Confirm` to save the script. <img src= "/customer-portal/img/cc46new.png">
...
7. **Leadset**: This section has information on the Leadsets. You can also `Create Leadset` . <img src= "/customer-portal/img/cc21new.png">
8. A new window will appear, prompting you to enter the required information:
   1. Enter the leadset `Name`.
   2. Pen-down any required `Description`.
   3. Select the `Campaign` from the drop-down for which you wish to associate the leadset.
   4. **State**: Configure the leadset’s operational status by setting it to either Active (available for assignment and dialing) or Inactive (disabled or paused).
   5. Click on `Confirm` to create the leadset. <img src= "/customer-portal/img/cc48new.png">
   6. Click on `See More` in the `Details` column.
   7. **A Leadset Dashboard** monitoring window will appear which displays the metrics of the leadset. <img src= "/customer-portal/img/cc49new.png">
   8.  Description of the fields in the stats window:
       1. `Status`: Set the leadset to `Active` or `Inactive` state.
       2.  Refresh your leadset parameters using `Refresh Lead Stats`.
       3.  Click on `Leadset Data` to view the leadset data.
       4.  Click on `Upload Leads` to upload the leadset data.  <img src= "/customer-portal/img/cc50new.png">
       5.  Upload your data via `File`, `URL` or by entering the `Text`.
          1. **File**: Drag and Drop a file or click to upload the `.csv`. <img src= "/customer-portal/img/cc51new.png">
             1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc52new.png">
             2. `Set row`: To indicate where the upload should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">
             3. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc28new.png"> <img src="/customer-portal/img/cc29new.png">
             4. Upon successful upload, the leadset will appear as shown below. <img src="/customer-portal/img/cc56new.png">
          2. **Uploading via URL**: Enter the URL in the CSV format and hit the Upload button. <img src="/customer-portal/img/cc57new.png">
             1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc52new.png">
             2. `Set row`: To indicate where the upload should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">
             3. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc28new.png"><img src="/customer-portal/img/cc29new.png">
             4. The successful upload can be seen in the image. <img src="/customer-portal/img/cc56.png">
             5. The updated `Leadset Dashboard` can be seen as. <img src="/customer-portal/img/cc87.png">
          ...
          3. **Uploading via Text**:
            ...
            44119, Ready
            ```
            1. Click on `Upload`. <img src="/customer-portal/img/cc58.png">
            2. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc52new.png">
            3. `Set row`: To indicate where the upload should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc27new.png">
            4. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc28new.png"> <img src="/customer-portal/img/cc.png">
            5. The successful upload can be seen in the image. <img src="/customer-portal/img/cc56.png">
            6. The updated `Leadset Dashboard` can be seen as. <img src="/customer-portal/img/cc87.png">

9.  `Continue to Modify`: If you click here, it will redirect to ConnexCS's Data Sure. [Click here](https://docs.connexcs.com/datasuite/) to know more.
10. **Edit Leadset**: Modify the details of an existing Leadset. The system will present the same interface as `Create Leadset`.
11. **Add Lead (Form)**: Manually add a lead via a form. <img src="/customer-portal/img/cc31.png">
12. **Lead Status**: Displays whether a lead is available, being worked on, needs follow-up, or is no longer active.
    1. `Total Leads`: The total number of leads uploaded or assigned to the campaign or leadset.
    2. `Dialable Leads`: Leads that are ready to be called (i.e., they're active, have valid numbers, and meet campaign rules).
    3. `Data Recycle`: Recycles leads that didn’t connect (e.g., no answer, busy) by returning them to the dial queue and updating their status based on retry rules.
13. **Call Status**: Displays counts for different call outcomes; like `Call Answered`, `Failed Calls`, `Call In Progress`, `Call Ringing`, `Dropped Calls`.
14. **Disposition Status**: Shows a pie chart for all the call disposition outcomes. Each colored section of the pie chart corresponds to one of these dispositions.
15. **Lead Agents Activity**: It displays performance stats of individual agents. The metrics include `Call Transfered`, `Busy`, `Call Back`, `Do Not Call`, `Not interested`, `Null`, `Sale made`, `Wrong details` etc.
...
3. In the `Campaign` tab, click `Add Campaign`. <img src= "/customer-portal/img/cc45.png">
4. A Window will pop-up with two sections:
   ...
   2. **Config**:
      ...
      5. Click on the `Confirm` button. <img src= "/customer-portal/img/cc66new.png">

   3. Click on `See More` in the `Details` column.
      1. A `Campaign Overview` will open. This interface is used for real-time monitoring of campaigns, lead stats, agent statuses, and calling activity. <img src= "/customer-portal/img/cc88new.png">
      2. `State`: Whether the campaign is `Active` or `Inactive`.
      3. `Edit Campaign`: Open the campaign settings for editing.
      4. `Back`: Return to the previous page.
      5. `Campaign Stats`: Provides real-time visual insights into the current status of the campaign's calling activities and agent activities.
...
9. **Leadset**: This section has information on the Leadsets. You can also `Create Leadset` . <img src= "/customer-portal/img/cc21new.png">
   1. A new window will appear, prompting you to enter the required information:
         ...
         5. Click on `Confirm` to create the leadset. <img src= "/customer-portal/img/cc67new.png">

         6. Click on `See More` in the `Details` column.

         7. **A Leadset Dashboard** monitoring window will appear which displays the metrics of the leadset. <img src= "/customer-portal/img/cc69new.png">

         8. Description of the fields in the stats window:
            1. `Status`: Set the leadset to `Active` or `Inactive` state.
            2. Refresh your leadset parameters using `Refresh Lead Stats`.
            3. Click on `Leadset Data` to view the leadset data. <img src= "/customer-portal/img/cc68new.png">
            4. Click on `Upload Leads` to upload the leadset data.
            5. Upload your data via `File`, `URL` or by entering the `Text`.
               1. **File**: Drag and Drop a file or click to upload the `.csv`. <img src= "/customer-portal/img/cc70new.png">
                  1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc71new.png">
                  2. `Set row`: To indicate where the upload should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc72new.png">
                  3. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc73new.png"> <img src="/customer-portal/img/cc74new.png">
                  4. The leadset gets uploaded sucessfully. <img src="/customer-portal/img/cc75new.png">
                  5. The leadset dasboard displays the total uploaded leads and dialable leads as well. <img src="/customer-portal/img/cc76.png">
               2. **Uploading via URL**: Enter the URL in the CSV format and hit the Upload button. <img src="/customer-portal/img/cc77new.png">
                     1. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc71new.png">
                     2. `Set row`: To indicate where the uploaad should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc72new.png">
                     3. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Phone` and `Status`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc73new.png"> <img src="cc74new.png">
                     4. The leadset gets uploaded sucessfully. <img src="/customer-portal/img/cc75new.png">
               ...
               3. **Uploading via Text**:
                  ...
                  2. Click on `Upload`. <img src="/customer-portal/img/cc77new.png">
                  3. A new window will open where two actions must be completed to create the leadset. <img src="/customer-portal/img/cc71new.png">
                  4. `Set row`: To indicate where the uploaad should start. `Right-click` on the row to want to start the uploading and click on `Set Start Row`. <img src="/customer-portal/img/cc72new.png">
                  5. `Map Table Columns`: Refers to the process of aligning or linking columns from one table to another, ensuring data consistency and accurate integration. This is essential when importing, migrating, or syncing data between different databases or systems. `Right-click` on the column you wish to be mapped, click on `Map Column` then `Key`. Click on `Upload`. This will upload the leadset data to the server. <img src="/customer-portal/img/cc73new.png"> <img src="/customer-portal/img/cc74new.png">
                  6. The successful upload can be seen in the image. <img src="/customer-portal/img/cc75new.png">
...
3. An Agent Dialer interface with the following fields will appear:
      ...
      4. `Logout` for the agent to log off from the system. <img src="/customer-portal/img/cc39new.png">
      ...
      9. `Call`: Hit the call button to phone the customer. <img src="/customer-portal/img/cc40new.png">
      10. When your agent hits the `Call` button, a `Live Call` window will appear. <img src="/customer-portal/img/cc41new.png">
...
4. A window will prompt you to either `Set Disposition` or `Update User Details`.  <img src="cc42.png"> <img src="/customer-portal/img/cc43new.png">
...
7. The updated `Campaign Overview` can be seen. <img src="/customer-portal/img/cc84new.png">
8. The updated `Leadset Overview` can be seen. <img src="/customer-portal/img/cc85new.png">

#### Auto Dialer

1. Enter the domain as entered in the **IDE** in [Step-I](add link). The screen will prompt the agent to enter the `Username` and `Password`. Click on `Login`. <img src="/customer-portal/img/cc37.png">
2. Select the campaign `Auto Dialer` from the drop-down menu and click `Confirm`. <img src="/customer-portal/img/cc59new.png">
3. An Agent Dialer interface with the following fields will appear:
      ...
      5. `Logout` for the agent to log off from the system. <img src="/customer-portal/img/cc60new.png">
      ...
      8. A `Live Call` window will appear. <img src="/customer-portal/img/cc61new.png">
          1. `Hold`: Allows your agent to place the call on hold.
          2. `Transfer`: Transfer call to another agent or manager.
          3. `Mute`: Allows your agent to mute your call.
          4. `Park`: **Currently *NOT IN USE***.
          ...
          6. `Keypad`: Displays the number keypad.
          7. `End`: Allows your agent to disconnect the call.
4. After the call ends, your agent can either `Submit` the disposition and get ready for the next call or `Submit & Go On Pause`. <img src="/customer-portal/img/cc62new.png">
...
7. The updated `Campaign Overview` can be seen. <img src="/customer-portal/img/cc851new.png">
 8. The updated `Leadset Overview` can be seen. <img src="/customer-portal/img/cc8511new.png">
...
5. The call is live. <img src="/customer-portal/img/cc81new.png"> 
6. Here, the person has asked the virtual assistant to change the `status` to **The sale has been made**. The change can be seen as in the `Broadcast Leadset`. <img src="/customer-portal/img/cc83new.png">
7. The `Braodcast Campaign Overview` display is also updated. <img src="/customer-portal/img/cc831new.png">
